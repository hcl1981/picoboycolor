...
keyLeft = Pin(3, Pin.IN, Pin.PULL_UP) 
keyRight = Pin(1, Pin.IN, Pin.PULL_UP)
keyFire = Pin(28, Pin.IN, Pin.PULL_UP)
keyThrust = Pin(27, Pin.IN, Pin.PULL_UP) 
...
spi = SPI(0, baudrate=31250000, sck=Pin(CLK_PIN), mosi=Pin(DIN_PIN))
tft = st7789.ST7789(spi, 240, 280,
...
for i in range(0,240):
    tft.line(0,0,240,int(i*1.166),st7789.color565(255-i,255,i))
for i in range(0,240):
    tft.line(0,0,240-i,int(240*1.166),st7789.color565(255,255-i,i))
...
def main():
    ...
    while True:
        ...
        if ship_alive:
            ...
            if keyFire.value() == 0 and len(missiles) < missile_max:
